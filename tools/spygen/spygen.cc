...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < ranks; ++i) {
        ...
        while (!feof(rankInfo)) {
            ...
            if (verbose) {
                printf(
                    "Attributing information: partner rank %d, send count=%s, send bytes=%s...\n",
    ...
    if (strcmp(imageFile, "-") == 0) {
        ...
        for (int i = 0; i < ranks; ++i) {
            for (int j = 0; j < ranks; ++j) {
                max_send_count =
                    getCommData(i, j) > max_send_count ? getCommData(i, j) : max_send_count;
                max_bytes_count =
                    getBytesData(i, j) > max_bytes_count ? getBytesData(i, j) : max_bytes_count;
            }
        }
                        ...
                        if (outputMode == 0) {
                            if (getCommData(i, j) == 0) {
                                ...
                                switch (outputType) {
                                    case IMAGE_BINARY:
                                        commBlock[0] = 0;
                                        commBlock[1] = (char) (255.0 * (getCommData(i, j) /
                                                                        ((double) max_send_count)));
                                        commBlock[2] = 0;
                                    ...
                                    case IMAGE_TEXT:
                                        fprintf(imagePPM, "%d %d %d ", 0,
                                                (int) (255.0 * (getCommData(i, j) /
                                                                ((double) max_send_count))), 0);
                                        break;
                                }
                            }
                            ...
                            if (getBytesData(i, j) == 0) {
                                ...
                                switch (outputType) {
                                    case IMAGE_BINARY:
                                        commBlock[0] = 0;
                                        commBlock[1] = (char) (255.0 * (getBytesData(i, j) /
                                                                        ((double) max_bytes_count)));
                                        commBlock[2] = 0;
                                    ...
                                    case IMAGE_TEXT:
                                        fprintf(imagePPM, "%d %d %d ", 0,
                                                (int) (255.0 * (getBytesData(i, j) /
                                                                ((double) max_bytes_count))), 0);
                                        break;
                                }
                            }
                        }
