    ...
    namespace Ember {
        ...
        public:
            EmberFamScatter_Event(Shmem::Interface &api, Output *output,
                                  Hermes::Vaddr src, Shmem::Fam_Descriptor fd, uint64_t nblocks,
                                  uint64_t firstBlock, uint64_t stride,
                                  uint64_t blockSize, bool blocking,
                                  EmberEventTimeStatistic *stat = NULL) :
                EmberFamEvent(api, output, stat), m_src(src), m_fd(fd), m_nblocks(nblocks),
            ...
            void issue(uint64_t time, Shmem::Callback callback) {
                ...
                m_api.fam_scatter(m_src, m_fd, m_nblocks, m_firstBlock, m_stride, m_blockSize,
                                  m_blocking, callback);
            }
