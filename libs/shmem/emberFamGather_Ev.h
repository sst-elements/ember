...
#ifndef _H_EMBER_FAM_GATHER_EVENT
#define _H_EMBER_FAM_GATHER_EVENT
...
namespace SST {
    namespace Ember {
        ...
        class EmberFamGather_Event : public EmberFamEvent {

        public:
            EmberFamGather_Event(Shmem::Interface &api, Output *output,
                                 Hermes::Vaddr dest, Shmem::Fam_Descriptor fd, uint64_t nblocks,
                                 uint64_t firstBlock, uint64_t stride,
            ...
            ~EmberFamGather_Event() {}

            std::string getName() { return "FamGather"; }

            void issue(uint64_t time, Shmem::Callback callback) {

                EmberEvent::issue(time);
                m_api.fam_gather(m_dest, m_fd, m_nblocks, m_firstBlock, m_stride, m_blockSize,
                                 m_blocking, callback);
            }
