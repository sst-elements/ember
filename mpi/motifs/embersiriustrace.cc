...
void EmberSIRIUSTraceGenerator::readMPICommSplit(std::queue<EmberEvent *> &evQ) {
    ...
    if (checkCommMapping != communicatorMap.end()) {
        fatal(CALL_INFO, -1, "Error: communicator mapped to %"
        PRIu32
        " already in comm map.\n", newCommID);
    }
...
void EmberSIRIUSTraceGenerator::readMPIIsend(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Isend to %"
    PRId32
    ", tag=%"
    PRId32
    ", count=%"
    ...
    if (checkReq == liveRequests.end()) {
        fatal(CALL_INFO, -1,
              "Error: when issuing an Isend, found an MPI_Request was already active.\n");
    }
...
void EmberSIRIUSTraceGenerator::readMPIRecv(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Recv from %"
    PRId32
    ", tag=%"
    PRId32
    ", count=%"
...
void EmberSIRIUSTraceGenerator::readMPIReduce(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Reduce count=%"
    PRIu32
    ", root=%"
    PRId32
    "\n", count, root);
...
void EmberSIRIUSTraceGenerator::readMPIAllreduce(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Allreduce count=%"
    PRIu32
    "\n", count);
...
void EmberSIRIUSTraceGenerator::readMPIIrecv(std::queue<EmberEvent *> &evQ) {
    ...
    if (checkReq != liveRequests.end()) {
        printLiveRequestMap();
        fatal(CALL_INFO, -1,
              "Error: when issuing an Irecv, found an MPI_Request was already active. (Request=%"
        PRIu64
        ")\n", req);
    }

    verbose(CALL_INFO, 2, 0, "Irecv src=%"
    PRId32
    ", count=%"
    PRIu32
    "\n", src, count);
...
void EmberSIRIUSTraceGenerator::readMPIWaitall(std::queue<EmberEvent *> &evQ) {
    ...
    MessageRequest *reqs = (MessageRequest *) malloc(
        sizeof(MessageRequest * ) * requestAddr.size());
    for (uint32_t i = 0; i < requestAddr.size(); i++) {
        auto findReq = liveRequests.find(requestAddr[i]);

        if (findReq == liveRequests.end()) {
            fatal(CALL_INFO, -1, "Error: unable to find request at address: %"
            PRIu64
            "\n", requestAddr[i]);
        } else {
    ...
    verbose(CALL_INFO, 2, 0, "Waitall, count=%"
    PRIu32
    ", found %"
    PRIu32
    " non MPI_REQUEST_NULL requests.\n",
...
void EmberSIRIUSTraceGenerator::readMPIWait(std::queue<EmberEvent *> &evQ) {
    ...
    if (SIRIUS_MPI_REQUEST_NULL != request) {
        ...
        if (reqLookup == liveRequests.end()) {
            fatal(CALL_INFO, -1,
                  "Error: unable to find a pending matching request for an MPI_Wait event.\n");
        }

        emberReq = reqLookup->second;

        verbose(CALL_INFO, 2, 0, "Wait, request=%"
        PRIu64
        "\n", request);
        ...
        verbose(CALL_INFO, 2, 0,
                "Wait: did not enqueue request because request entry is MPI_REQUEST_NULL\n");
    }
}
...
void EmberSIRIUSTraceGenerator::readMPIBcast(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Bcast: root=%"
    PRId32
    ", count=%"
    PRIu32
    "\n", root, count);
...
double EmberSIRIUSTraceGenerator::readTime() const {
    ...
    if (1 != readLen) {
        fatal(CALL_INFO, -1, "I/O Error reading from SIRIUS trace, size read %"
        PRIu64
        " != %"
        PRIu64
        "\n",
            (uint64_t) readLen, (uint64_t)
        sizeof(tmp));
    }
...
const Communicator *EmberSIRIUSTraceGenerator::readCommunicator() const {
    ...
    if (0 == comm) {
        ...
        if (communicatorMap.end() == findComm) {
            ...
            fatal(CALL_INFO, -1, "Unknown communicator found in trace (comm=%"
            PRIu32
            ")\n",
                comm);
        }
