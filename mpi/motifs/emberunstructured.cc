...
EmberUnstructuredGenerator::EmberUnstructuredGenerator(SST::Component *owner, Params &params) :
    ...
    if (graphFile.compare("-1") == 0)
        fatal(CALL_INFO, -1,
              "Communication graph file must be specified for unstructured motifs!\n");
...
void EmberUnstructuredGenerator::configure() {
    ...
    if (0 == rank()) {
        output("Unstructured motif problem size: %"
        PRIu32
        "\n", p_size);
        output("Unstructured motif compute time: %"
        PRIu32
        " ns\n", nsCompute);
        output("Unstructured motif iterations: %"
        PRIu32
        "\n", iterations);
        output("Unstructured motif iterms/cell: %"
        PRIu32
        "\n", items_per_cell);
    ...
    if (0 == rank()) {
        for (unsigned int i = 0; i < rawCommMap->size(); i++) {
            ...
            for (std::map<int, int>::iterator it = rawCommMap->at(i).begin();
                 it != rawCommMap->at(i).end(); it++) {
    ...
    if (use_CustomRankMap) {
        ...
        for (unsigned int i = 0; i < CommMap->size(); i++) {
            ...
            for (std::map<int, int>::iterator it = rawCommMap->at(i).begin();
                ...
                CommMap->at(
                    srcTask)[destTask] = 1; //1 could be changed to the weight (it->second) in the future
                //if(0 == rank())
...
bool EmberUnstructuredGenerator::generate(std::queue<EmberEvent *> &evQ) {
    ...
    if (m_loopIndex == iterations) {
        ...
        if (0 == rank()) {
            ...
            output("Motif Latency: JobNum:%d Per iteration latency:%.3f us\n", jobId,
                   latency_per_iter);
            output("Job Finished: JobNum:%d Time:%"
            PRIu64
    ...
    for (std::map<int, int>::iterator it = CommMap->at(rank()).begin();
         it != CommMap->at(rank()).end(); it++) {
        MessageRequest *req = new MessageRequest();
        requests.push_back(req);

        //std::cout << rank() << " communicates with " << it->first << std::endl;
        enQ_irecv(evQ, (int32_t) it->first, items_per_cell * sizeof_cell * p_size, 0, GroupWorld,
                  req);
...
std::vector <std::map<int, int>> *EmberUnstructuredGenerator::readCommFile(std::string fileName,
                                                                           int procsNeeded) {
    //read matrix
    MatrixMarketReader2D<int> reader = MatrixMarketReader2D<int>();
    std::vector<int *> *dataVec = reader.readMatrix(fileName.c_str(),
                                                    true); //current version ignores edge weights
