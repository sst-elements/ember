...
void EmberHalo2DNBRGenerator::completed(const SST::Output *output, uint64_t) {
    output->verbose(CALL_INFO, 2, 0, "Generator finishing, sent: %"
    PRIu32
    " messages.\n", messageCount);
}
...
void EmberHalo2DNBRGenerator::configure() {
    ...
    verbose(CALL_INFO, 2, 0, "Processor grid dimensions, X=%"
    PRIu32
    ", Y=%"
    PRIu32
    "\n",
...
bool EmberHalo2DNBRGenerator::generate(std::queue<EmberEvent *> &evQ) {
    ...
    verbose(CALL_INFO, 2, 0, "Halo 2DNBR motif generating events for loopIndex %"
    PRIu32
    "\n", m_loopIndex);
    ...
    verbose(CALL_INFO, 2, 0, "Halo 2DNBR motif completed event generation for loopIndex: %"
    PRIu32
    "\n", m_loopIndex);
