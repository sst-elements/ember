...
bool EmberDetailedRingGenerator::generate(std::queue<EmberEvent *> &evQ) {
        ...
        if (m_printRank == rank() || -1 == m_printRank) {
            ...
            if (m_computeFunc) {
                ...
                output("%s rank %d, `%s` total compute %.3f us\n", getMotifName().c_str(), rank(),
                       m_computeFunc == &EmberDetailedRingGenerator::computeSimple ? "Simple"
                                                                                   : "Detailed",
    ...
    verbose(CALL_INFO, 2, 1, "sendbuff=%"
    PRIx64
    "\n", m_sendBuf.getSimVAddr());
    verbose(CALL_INFO, 2, 1, "recvbuff=%"
    PRIx64
    "\n", m_recvBuf.getSimVAddr());
    verbose(CALL_INFO, 2, 1, "streambuff=%"
    PRIx64
    ...
    if (0 == m_loopIndex) {
        ...
        if (m_printRank == rank() || -1 == m_printRank) {
            if (m_computeTime) {
                output("%s rank %d, 'Simple' computeTime=%"
                PRIu64
                " computeWindow=%"
                PRIu64
                "\n",
