...
EmberHalo3D26Generator::EmberHalo3D26Generator(SST::Component *owner, Params &params) :
    ...
    if (params.contains("arg.computetime")) {
        ...
        uint64_t total_flops =
            total_grid_points * ((uint64_t) items_per_cell) * ((uint64_t) flops_per_cell);
    ...
    if (peX == 0 || peY == 0 || peZ == 0) {
        ...
        uint32_t varExisting = (uint32_t)(((peX * peX) + (peY * peY) + (peZ * peZ)) / 3) -
                               (meanExisting * meanExisting);
            ...
            for (int32_t j = 0; j < size(); j++) {
                for (int32_t k = 0; k < size(); k++) {
                    if ((i * j * k) == size()) {
                        ...
                        const uint32_t varNew =
                            (((i * i) + (j * j) + (k * k)) / 3) - (meanNew * meanNew);

                        if (varNew <= varExisting) {
                            if (0 == rank()) {
                                verbose(CALL_INFO, 2, 0,
                                        "Found an improved decomposition solution: %"
                                PRIu32
                                " x %"
                                PRIu32
                                " x %"
                            ...
                            varExisting =
                                (uint32_t)(((peX * peX) + (peY * peY) + (peZ * peZ)) / 3) -
                                (meanExisting * meanExisting);
                        }
                    }
                }
    ...
    if (0 == rank()) {
        output("Halo3D processor decomposition solution: %"
        PRIu32
        "x%"
        PRIu32
        "x%"
        ...
        PRIu32
        "\n", nx, ny, nz);
        output("Halo3D compute time: %"
        PRIu64
        " ns\n", compute_the_time );
        output("Halo3D copy time:    %"
        PRIu32
        " ns\n", nsCopyTime);
        output("Halo3D iterations:   %"
        PRIu32
        "\n", iterations);
        output("Halo3D items/cell:   %"
        PRIu32
        "\n", items_per_cell);
        output("Halo3D do reduction: %"
        PRIu32
        "\n", performReduction);
    }
    ...
    verbose(CALL_INFO, 2, 0, "Rank: %"
    PRIu32
    ", Total communication partners: %d\n", rank(), (int) requestLength);
    verbose(CALL_INFO, 2, 0, "Rank: %"
    PRIu32
    ", X+: %"
    PRId32
...
bool EmberHalo3D26Generator::generate(std::queue<EmberEvent *> &evQ) {
    verbose(CALL_INFO, 1, 0, "Iteration on rank %"
    PRId32
    "\n", rank());
    ...
    if (xface_down > -1) {
        enQ_irecv(evQ, xface_down, items_per_cell * sizeof_cell * ny * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (xface_up > -1) {
        enQ_irecv(evQ, xface_up, items_per_cell * sizeof_cell * ny * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (yface_down > -1) {
        enQ_irecv(evQ, yface_down, items_per_cell * sizeof_cell * nx * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (yface_up > -1) {
        enQ_irecv(evQ, yface_up, items_per_cell * sizeof_cell * nx * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (zface_down > -1) {
        enQ_irecv(evQ, zface_down, items_per_cell * sizeof_cell * ny * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (zface_up > -1) {
        enQ_irecv(evQ, zface_up, items_per_cell * sizeof_cell * ny * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_a > -1) {
        enQ_irecv(evQ, line_a, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_b > -1) {
        enQ_irecv(evQ, line_b, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_c > -1) {
        enQ_irecv(evQ, line_c, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_d > -1) {
        enQ_irecv(evQ, line_d, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_e > -1) {
        enQ_irecv(evQ, line_e, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_f > -1) {
        enQ_irecv(evQ, line_f, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_g > -1) {
        enQ_irecv(evQ, line_g, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_h > -1) {
        enQ_irecv(evQ, line_h, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_i > -1) {
        enQ_irecv(evQ, line_i, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_j > -1) {
        enQ_irecv(evQ, line_j, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_k > -1) {
        enQ_irecv(evQ, line_k, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_l > -1) {
        enQ_irecv(evQ, line_l, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_a > -1) {
        enQ_irecv(evQ, corner_a, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_b > -1) {
        enQ_irecv(evQ, corner_b, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_c > -1) {
        enQ_irecv(evQ, corner_c, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_d > -1) {
        enQ_irecv(evQ, corner_d, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_e > -1) {
        enQ_irecv(evQ, corner_e, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_f > -1) {
        enQ_irecv(evQ, corner_f, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_g > -1) {
        enQ_irecv(evQ, corner_g, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_h > -1) {
        enQ_irecv(evQ, corner_h, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }
    ...
    // Enqueue the sends
    if (xface_down > -1) {
        enQ_isend(evQ, xface_down, items_per_cell * sizeof_cell * ny * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (xface_up > -1) {
        enQ_isend(evQ, xface_up, items_per_cell * sizeof_cell * ny * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (yface_down > -1) {
        enQ_isend(evQ, yface_down, items_per_cell * sizeof_cell * nx * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (yface_up > -1) {
        enQ_isend(evQ, yface_up, items_per_cell * sizeof_cell * nx * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (zface_down > -1) {
        enQ_isend(evQ, zface_down, items_per_cell * sizeof_cell * ny * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (zface_up > -1) {
        enQ_isend(evQ, zface_up, items_per_cell * sizeof_cell * ny * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_a > -1) {
        enQ_isend(evQ, line_a, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_b > -1) {
        enQ_isend(evQ, line_b, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_c > -1) {
        enQ_isend(evQ, line_c, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_d > -1) {
        enQ_isend(evQ, line_d, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_e > -1) {
        enQ_isend(evQ, line_e, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_f > -1) {
        enQ_isend(evQ, line_f, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_g > -1) {
        enQ_isend(evQ, line_g, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_h > -1) {
        enQ_isend(evQ, line_h, items_per_cell * sizeof_cell * ny, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_i > -1) {
        enQ_isend(evQ, line_i, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_j > -1) {
        enQ_isend(evQ, line_j, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_k > -1) {
        enQ_isend(evQ, line_k, items_per_cell * sizeof_cell * nz, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (line_l > -1) {
        enQ_isend(evQ, line_l, items_per_cell * sizeof_cell * nx, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_a > -1) {
        enQ_isend(evQ, corner_a, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_b > -1) {
        enQ_isend(evQ, corner_b, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_c > -1) {
        enQ_isend(evQ, corner_c, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_d > -1) {
        enQ_isend(evQ, corner_d, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_e > -1) {
        enQ_isend(evQ, corner_e, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_f > -1) {
        enQ_isend(evQ, corner_f, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_g > -1) {
        enQ_isend(evQ, corner_g, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }

    if (corner_h > -1) {
        enQ_isend(evQ, corner_h, items_per_cell * sizeof_cell * 1, 0, GroupWorld,
                  &requests[nextRequest]);
        nextRequest++;
    }
    ...
    verbose(CALL_INFO, 1, 0, "Iteration on rank %"
    PRId32
    " completed generation, %d events in queue\n",
        rank(), (int) evQ.size());
