    ...
    switch (m_InnerLoopIndex) {
        ...
        case 0:
            ...
            for (uint32_t i = 0; i < nz; i += kba) {
                if (x_down >= 0) {
                    enQ_recv(evQ, x_down, (nx * kba * data_width * fields_per_cell), 1000,
                             GroupWorld);
                }

                if (y_down >= 0) {
                    enQ_recv(evQ, y_down, (ny * kba * data_width * fields_per_cell), 1000,
                             GroupWorld);
                }
                ...
                if (x_up >= 0) {
                    enQ_send(evQ, x_up, (nx * kba * data_width * fields_per_cell), 1000,
                             GroupWorld);
                }

                if (y_up >= 0) {
                    enQ_send(evQ, y_up, (ny * kba * data_width * fields_per_cell), 1000,
                             GroupWorld);
                }
            }
        ...
        case 1:
            ...
            for (uint32_t i = 0; i < nz; i += kba) {
                if (x_up >= 0) {
                    enQ_recv(evQ, x_up, (nx * kba * data_width * fields_per_cell), 2000,
                             GroupWorld);
                }

                if (y_down >= 0) {
                    enQ_recv(evQ, y_down, (ny * kba * data_width * fields_per_cell), 2000,
                             GroupWorld);
                }
                ...
                if (x_down >= 0) {
                    enQ_send(evQ, x_down, (nx * kba * data_width * fields_per_cell), 2000,
                             GroupWorld);
                }

                if (y_up >= 0) {
                    enQ_send(evQ, y_up, (ny * kba * data_width * fields_per_cell), 2000,
                             GroupWorld);
                }
            }
        ...
        case 2:
            ...
            for (uint32_t i = 0; i < nz; i += kba) {
                if (x_up >= 0) {
                    enQ_recv(evQ, x_up, (nx * kba * data_width * fields_per_cell), 3000,
                             GroupWorld);
                }

                if (y_up >= 0) {
                    enQ_recv(evQ, y_up, (ny * kba * data_width * fields_per_cell), 3000,
                             GroupWorld);
                }
                ...
                if (x_down >= 0) {
                    enQ_send(evQ, x_down, (nx * kba * data_width * fields_per_cell), 3000,
                             GroupWorld);
                }

                if (y_down >= 0) {
                    enQ_send(evQ, y_down, (ny * kba * data_width * fields_per_cell), 3000,
                             GroupWorld);
                }
            }
        ...
        case 3:
            ...
            for (uint32_t i = 0; i < nz; i += kba) {
                if (x_down >= 0) {
                    enQ_recv(evQ, x_down, (nx * kba * data_width * fields_per_cell), 4000,
                             GroupWorld);
                }

                if (y_up >= 0) {
                    enQ_recv(evQ, y_up, (ny * kba * data_width * fields_per_cell), 4000,
                             GroupWorld);
                }
                ...
                if (x_up >= 0) {
                    enQ_send(evQ, x_up, (nx * kba * data_width * fields_per_cell), 4000,
                             GroupWorld);
                }

                if (y_down >= 0) {
                    enQ_send(evQ, y_down, (ny * kba * data_width * fields_per_cell), 4000,
                             GroupWorld);
                }
            }
