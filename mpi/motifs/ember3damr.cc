...
Ember3DAMRGenerator::Ember3DAMRGenerator(SST::Component *owner, Params &params) :
    ...
    out->verbose(CALL_INFO, 2, 0, "Block sizes are X=%"
    PRIu32
    ", Y=%"
    PRIu32
    ", Z=%"
    ...
    out->verbose(CALL_INFO, 2, 0, "Motif will run %"
    PRIu32
    " iterations\n", maxIterations);
...
//        amrFile = new EmberAMRTextFile(blockFilePath, out);
        out->fatal(CALL_INFO, -1,
                   "Binary mesh files are the only type currently supported, use sst-meshconvert\n");
    }
    ...
    out->verbose(CALL_INFO, 2, 0, "Loaded AMR block information: %"
    PRIu32
    " blocks, %"
    PRIu32
    " max refinement, blocks (X=%"
    PRIu32
    ",Y=%"
    PRIu32
    ",Z=%"
...
//	int32_t  zDown = 0;
    ...
    out->verbose(CALL_INFO, 2, 0, "Rank %"
    PRIu32
    ", loaded %"
    PRIu32
    " blocks locally and %"
    PRIu32
    " remotely, stopped at line: %"
    PRIu32
    ".\n", (uint32_t) rank(),
...
//	}
    ...
    for (uint32_t i = 0; i < localBlocks.size(); ++i) {
        ...
        out->verbose(CALL_INFO, 16, 0, "Wiring block %"
        PRIu32
        ...
        calcBlockLocation(currentBlock->getBlockID(), blockLevel, &blockXPos, &blockYPos,
                          &blockZPos);

        out->verbose(CALL_INFO, 16, 0, "Block is refinement level %"
        PRIu32
        ", located at (%"
        PRIu32
        ", %"
        ...
        out->verbose(CALL_INFO, 16, 0, "Block %"
        PRIu32
        " has X+:%"
        PRId32
        ...
        if (blockXUp == -2) {
            ...
            if (blockNode == blockToNodeMap.end() && isBlockLocal(commToBlock)) {
                if (!isBlockLocal(commToBlock)) {
                    out->fatal(CALL_INFO, -1, "Could not locate block %"
                    PRIu32
                    ", during wire up phase.\n", commToBlock);
                }
            } else {
            ...
            const uint32_t x1 = calcBlockID(blockXPos * 2 + 2, blockYPos * 2, blockZPos * 2,
                                            blockXUp);
            const uint32_t x2 = calcBlockID(blockXPos * 2 + 2, blockYPos * 2 + 1, blockZPos * 2,
                                            blockXUp);
            const uint32_t x3 = calcBlockID(blockXPos * 2 + 2, blockYPos * 2, blockZPos * 2 + 1,
                                            blockXUp);
            const uint32_t x4 = calcBlockID(blockXPos * 2 + 2, blockYPos * 2 + 1, blockZPos * 2 + 1,
                                            blockXUp);
            ...
            if (blockNodeX1 == blockToNodeMap.end()) {
                if (isBlockLocal(x1)) {
                    ...
                    out->fatal(CALL_INFO, -1,
                               "X+ wireup for block failed to locate wire up partner (block: %"
                    PRIu32
                    ")\n", x1);
                }
            }
            ...
            out->verbose(CALL_INFO, 32, 0, "Mapping block %"
            PRIu32
            " to nodes %"
            PRId32
            ", %"
            ...
            std::map<uint32_t, int32_t>::iterator blockNextToMeNode = blockToNodeMap.find(
                blockNextToMe);

            if (blockNextToMeNode == blockToNodeMap.end()) {
                if (!isBlockLocal(blockNextToMe)) {
                    out->fatal(CALL_INFO, -1,
                               "X+ wireup for block failed to locate wire up on same refinement level (block=%"
                    PRIu32
                    "\n",
                        blockNextToMe);
                }
        ...
        if (blockXDown == -2) {
            ...
            out->verbose(CALL_INFO, 16, 0, "Block %"
            PRIu32
            " calculating X down with refinement, (%"
            PRIu32
            "/2-1=%d"
            PRId32
            ", %"
            ...
            // Communication to a finer level (my refinement is less than block next to me)
            const uint32_t x1 = calcBlockID(blockXPos * 2 - 1, blockYPos * 2, blockZPos * 2,
                                            blockXDown);
            const uint32_t x2 = calcBlockID(blockXPos * 2 - 1, blockYPos * 2 + 1, blockZPos * 2,
                                            blockXDown);
            const uint32_t x3 = calcBlockID(blockXPos * 2 - 1, blockYPos * 2, blockZPos * 2 + 1,
                                            blockXDown);
            const uint32_t x4 = calcBlockID(blockXPos * 2 - 1, blockYPos * 2 + 1, blockZPos * 2 + 1,
                                            blockXDown);
            ...
            if (blockNextToMeNode == blockToNodeMap.end()) {
                if (!isBlockLocal(blockNextToMe)) {
                    out->fatal(CALL_INFO, -1,
                               "X- wireup for block failed to locate wire up block on same refinment level (block: %"
                    PRIu32
                    ")\n", blockNextToMe);
                }
            } else {
        ...
        if (blockYUp == -2) {
            ...
            // Communication to a finer level (my refinement is less than block next to me)
            const uint32_t y1 = calcBlockID(blockXPos * 2, blockYPos * 2 + 2, blockZPos * 2,
                                            blockYUp);
            const uint32_t y2 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 + 2, blockZPos * 2,
                                            blockYUp);
            const uint32_t y3 = calcBlockID(blockXPos * 2, blockYPos * 2 + 2, blockZPos * 2 + 1,
                                            blockYUp);
            const uint32_t y4 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 + 2, blockZPos * 2 + 1,
                                            blockYUp);
            ...
            if (blockNodeY1 == blockToNodeMap.end()) {
                if (isBlockLocal(y1)) {
                    ...
                    out->fatal(CALL_INFO, -1,
                               "Y+ wireup for block failed to locate wire up partner (block: %"
                    PRIu32
                    ")\n", y1);
                }
            }
            ...
            if (blockNextToMeNode == blockToNodeMap.end()) {
                if (!isBlockLocal(blockNextToMe)) {
                    out->output("Dumping block map for rank: %"
                    PRIu32
                    "\n", rank());
                    out->fatal(CALL_INFO, -1,
                               "Y+ wireup for block failed to locate wire up partner (block: %"
        ...
        if (blockYDown == -2) {
            ...
            // Communication to a finer level (my refinement is less than block next to me)
            const uint32_t y1 = calcBlockID(blockXPos * 2, blockYPos * 2 - 1, blockZPos * 2,
                                            blockYDown);
            const uint32_t y2 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 - 1, blockZPos * 2,
                                            blockYDown);
            const uint32_t y3 = calcBlockID(blockXPos * 2, blockYPos * 2 - 1, blockZPos * 2 + 1,
                                            blockYDown);
            const uint32_t y4 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 - 1, blockZPos * 2 + 1,
                                            blockYDown);
        ...
        if (blockZUp == -2) {
            ...
            // Communication to a finer level (my refinement is less than block next to me)
            const uint32_t z1 = calcBlockID(blockXPos * 2, blockYPos * 2, blockZPos * 2 + 2,
                                            blockZUp);
            const uint32_t z2 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2, blockZPos * 2 + 2,
                                            blockZUp);
            const uint32_t z3 = calcBlockID(blockXPos * 2, blockYPos * 2 + 1, blockZPos * 2 + 2,
                                            blockZUp);
            const uint32_t z4 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 + 1, blockZPos * 2 + 2,
                                            blockZUp);
            ...
            if (blockNodeZ1 == blockToNodeMap.end()) {
                if (isBlockLocal(z1)) {
                    ...
                    out->fatal(CALL_INFO, -1,
                               "Z+ wireup for block failed to locate wire up partner (block: %"
                    PRIu32
                    ")\n", z1);
                }
            }
        ...
        if (blockZDown == -2) {
            ...
            // Communication to a finer level (my refinement is less than block next to me
            const uint32_t z1 = calcBlockID(blockXPos * 2, blockYPos * 2, blockZPos * 2 - 1,
                                            blockZDown);
            const uint32_t z2 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2, blockZPos * 2 - 1,
                                            blockZDown);
            const uint32_t z3 = calcBlockID(blockXPos * 2, blockYPos * 2 + 1, blockZPos * 2 - 1,
                                            blockZDown);
            const uint32_t z4 = calcBlockID(blockXPos * 2 + 1, blockYPos * 2 + 1, blockZPos * 2 - 1,
                                            blockZDown);
    ...
    out->verbose(CALL_INFO, 2, 0, "Maximum requests from rank %"
    PRIu32
    " will be set up: %"
    PRIu32
    ...
    blockMessageBuffer = memAlloc(
        sizeof(double) * maxFaceDim * maxFaceDim * localBlocks.size() * 2);

    out->verbose(CALL_INFO, 2, 0, "Blocks on rank %"
    PRIu32
    " count is: %"
    PRIu32
...
void Ember3DAMRGenerator::postBlockCommunication(std::queue<EmberEvent *> &evQ, int32_t *blockComm,
    ...
    for (uint32_t i = 0; i < 4; ++i) {
        if (blockComm[i] >= 0) {
            if (blockComm[i] != thisRank) {
                out->verbose(CALL_INFO, 16, 0, "Setting up exchange with rank %"
                PRId32
                " for message size: %"
                PRIu32
                ", tag: %"
                ...
                out->verbose(CALL_INFO, 32, 0, "Enqueue non-blocking recv from: %"
                PRIu32
                " on rank %"
                PRIu32
                ", size: %"
                ...
                if ((*nextReq) < maxRequestCount) {
                    enQ_irecv(evQ, &bufferPtr[(*nextReq) * maxFaceDim * maxFaceDim],
                              items_per_cell * faceSize, DOUBLE, blockComm[i], msgTag, GroupWorld,
                              &requests[(*nextReq)]);
                    (*nextReq) = (*nextReq) + 1;
                } else {
                    out->fatal(CALL_INFO, -1, "Error: max requests at: %"
                    PRIu32
                    ", current total: %"
                    PRIu32
                ...
                out->verbose(CALL_INFO, 32, 0, "Enqueue non-blocking send to: %"
                PRIu32
                " from rank %"
                PRIu32
                ", size: %"
...
void Ember3DAMRGenerator::aggregateCommBytes(Ember3DAMRBlock *curBlock,
...
bool Ember3DAMRGenerator::generate(std::queue<EmberEvent *> &evQ) {
    if (iteration < maxIterations) {
        ...
        out->verbose(CALL_INFO, 8, 0, "Executing iteration: %"
        PRIu32
        " on rank %"
        PRIu32
        ", local blocks count: %"
...
//		for(uint32_t i = 0; i < localBlocks.size(); ++i) {
        out->verbose(CALL_INFO, 8, 0, "Loading information for block: %"
        PRIu32
        " out of %"
        PRIu64
        "\n",
        ...
        out->verbose(CALL_INFO, 16, 0, "Creating communication events for block %"
        PRIu32
        "\n", currentBlock->getBlockID());

        out->verbose(CALL_INFO, 32, 0, "-> Processing X-Down direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommXDown(), &nextRequestID, blockNy * blockNz,
                               1001, currentBlock);

        out->verbose(CALL_INFO, 32, 0, "-> Processing X-Down direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommXUp(), &nextRequestID, blockNy * blockNz,
                               1001, currentBlock);

        out->verbose(CALL_INFO, 32, 0, "-> Processing Y-Down direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommYDown(), &nextRequestID, blockNx * blockNz,
                               2001, currentBlock);

        out->verbose(CALL_INFO, 32, 0, "-> Processing Y-Up direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommYUp(), &nextRequestID, blockNx * blockNz,
                               2001, currentBlock);

        out->verbose(CALL_INFO, 32, 0, "-> Processing Z-Down direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommZDown(), &nextRequestID, blockNx * blockNy,
                               4001, currentBlock);

        out->verbose(CALL_INFO, 32, 0, "-> Processing Z-Up direction...\n");
        postBlockCommunication(evQ, currentBlock->getCommZUp(), &nextRequestID, blockNx * blockNy,
                               4001, currentBlock);
...
//		}
        ...
        if (nextBlockToBeProcessed == localBlocks.size()) {
            if (nextRequestID > 0) {
                out->verbose(CALL_INFO, 2, 0, "Enqueued %"
                PRIu32
                " events, issuing wait all against them on rank %"
                PRIu32
                "\n",
                ...
                out->verbose(CALL_INFO, 2, 0,
                             "Enqueued no communication events, stepping over wait-all issue.\n");
            }
...
//		iteration++;
        ...
        sprintf(timeBuffer, "%"
        PRIu64
        "ns", getCurrentSimTimeNano());
        UnitAlgebra timeUA(timeBuffer);

        out->verbose(CALL_INFO, 2, 0, "Completed %"
        PRIu32
        " iterations @ time=%s, will now complete and unload.\n",
...
void Ember3DAMRGenerator::calcBlockLocation(const uint32_t blockID, const uint32_t blockLevel,
...
uint32_t Ember3DAMRGenerator::calcBlockID(const uint32_t posX, const uint32_t posY,
                                          const uint32_t posZ,
                                          const uint32_t level) {

    out->verbose(CALL_INFO, 32, 0, "Calculating block ID for position (%"
    PRIu32
    ", %"
    PRIu32
    ", %"
    ...
    out->verbose(CALL_INFO, 32, 0, "Start index of refinement level is: %"
    PRIu32
    "\n", startIndex);
    ...
    out->verbose(CALL_INFO, 32, 0, "Refinement %"
    PRIu32
    " blocks X=%"
    PRIu32
    ", Y=%"
    ...
    out->verbose(CALL_INFO, 32, 0, "Calculate block ID, startIndex=%"
    PRIu32
    " + %"
    PRIu32
    " + %"
    ...
    out->verbose(CALL_INFO, 32, 0, "Final block ID is %"
    PRIu32
    "\n", final_location);
...
void Ember3DAMRGenerator::printBlockMap() {
    ...
    sprintf(map_output, "blocks-local-%"
    PRIu32
    ".map", rank());
