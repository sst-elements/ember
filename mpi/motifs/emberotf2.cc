...
static OTF2_CallbackCode EmberOTF2LeaveRegion(
    ...
    gen->verbose(CALL_INFO, 4, 0, "Time-Difference: %"
    PRIu64
    "\n", timeDiff );
    gen->setCurrentTime(time);
...
static OTF2_CallbackCode EmberOTF2MPISend(
    ...
    EmberOTF2Generator *gen = (EmberOTF2Generator *) userData;
    gen->enQ_send(*(gen->getEventQueue()), 0, msgLen,
                  gen->extractDataTypeFromAttributeList(attributes), receiver, tag, GroupWorld);
...
static OTF2_CallbackCode EmberOTF2MPIISend(
    ...
    EmberOTF2Generator *gen = (EmberOTF2Generator *) userData;
    gen->enQ_isend(*(gen->getEventQueue()), 0, msgLen,
                   gen->extractDataTypeFromAttributeList(attributes), receiver, tag, GroupWorld,
                   newReq);
...
static OTF2_CallbackCode EmberOTF2MPIISendComplete(
    ...
    EmberOTF2Generator *gen = (EmberOTF2Generator *) userData;
    gen->verbose(CALL_INFO, 4, 0, "Trace::ISendComplete, reqID=%"
    PRIu64
    "\n", reqID );
    ...
    if (checkReq == gen->getRequestMap().end()) {
        gen->printRequestMap();
        gen->fatal(CALL_INFO, -1, "Error: search for request ID %"
        PRIu64
...
static OTF2_CallbackCode EmberOTF2MPIRecv(
    ...
    EmberOTF2Generator *gen = (EmberOTF2Generator *) userData;
    gen->verbose(CALL_INFO, 4, 0, "Trace::Recv sender=%"
    PRIu32
    ", tag=%"
    PRIu32
    ", len=%"
    ...
    gen->enQ_recv(*(gen->getEventQueue()), 0, msgLen,
                  gen->extractDataTypeFromAttributeList(attributes), sender, tag, GroupWorld);
...
static OTF2_CallbackCode EmberOTF2MPICollectiveEnd(
    ...
    EmberOTF2Generator *gen = (EmberOTF2Generator *) userData;
    gen->verbose(CALL_INFO, 4, 0, "Trace::CollectiveEnd (Triggers Start in Ember) from Root=%"
    PRIu32
    "\n",
        root );
    ...
    switch (collectiveOp) {
        ...
        case OTF2_COLLECTIVE_OP_BCAST:
            gen->verbose(CALL_INFO, 4, 0, "Trace::Bcast, Root=%"
            PRIu32
            ", Size=%"
            PRIu64
            "\n", root, sizeSent );
        ...
        case OTF2_COLLECTIVE_OP_ALLREDUCE:
            gen->verbose(CALL_INFO, 4, 0, "Trace::Allreduce, Root=%"
            PRIu32
            ", Size=%"
            PRIu64
            "\n",
        ...
        case OTF2_COLLECTIVE_OP_REDUCE:
            gen->verbose(CALL_INFO, 4, 0, "Trace::Reduce, Root=%"
            PRIu32
            ", Size=%"
            PRIu64
            "\n",
                root, sizeSent);
            gen->enQ_reduce(*(gen->getEventQueue()), 0, 0, sizeSent, CHAR, SUM,
        ...
        default:
            gen->verbose(CALL_INFO, 16, 0,
                         "Not supported collective operation, set to ignore event.\n");
            break;
    }
...
static OTF2_CallbackCode EmberOTF2RegisterLocation(void *userData,
                                                   OTF2_LocationRef location,
                                                   OTF2_StringRef locationName,
                                                   OTF2_LocationType locationType,
                                                   uint64_t locationEvents,
...
EmberOTF2Generator::EmberOTF2Generator(SST::Component *owner, Params &params) :
    ...
    verbose(CALL_INFO, 2, 0, "Found %"
    PRIu64
    " locations, motif is rank %"
    PRIu64
    ".\n", traceLocationCount,
        static_cast<uint64_t>(rank()));

    if (static_cast<uint64_t>(size()) != traceLocationCount) {
        verbose(CALL_INFO, 1, 0, "Error: trace contains %"
        PRIu64
        " locations, but the simulation has %"
        PRIu64
        " ranks.\n",
...
*/
    ...
    verbose(CALL_INFO, 1, 0, "There are: %"
    PRIu64
    " global definitions.\n", globalDefinitions );
    ...
    verbose(CALL_INFO, 1, 0, "Read %"
    PRIu64
    " global definitions from trace on rank %"
    PRIu64
    "\n",
    ...
    if (OTF2_SUCCESS != OTF2_Reader_SelectLocation(traceReader, static_cast<uint64_t>(rank()))) {
        fatal(CALL_INFO, -1, "Error: unable to select location %"
        PRIu64
        "\n", rank());
    }
    ...
    if (traceOpenedDefFiles) {
        ...
        if (NULL == traceLocalDefReader) {
            fatal(CALL_INFO, -1, "Error: unable to open a local definition reader on rank %"
            PRIu64
            "\n", rank());
        } else {
            verbose(CALL_INFO, 1, 0, "Opened local definition reader on rank %"
            PRIu64
            "\n", rank());
        }
    ...
    OTF2_GlobalEvtReaderCallbacks_SetProgramBeginCallback(traceGlobalEvtCallbacks,
                                                          EmberOTF2StartProgram);
    OTF2_GlobalEvtReaderCallbacks_SetProgramEndCallback(traceGlobalEvtCallbacks,
                                                        EmberOTF2EndProgram);
    ...
    OTF2_GlobalEvtReaderCallbacks_SetMpiIrecvRequestCallback(traceGlobalEvtCallbacks,
                                                             EmberOTF2MPIIRecvRequest);
    OTF2_GlobalEvtReaderCallbacks_SetMpiIsendCompleteCallback(traceGlobalEvtCallbacks,
                                                              EmberOTF2MPIISendComplete);
...
//	}
    ...
    if (hasEvent) {
        verbose(CALL_INFO, 4, 0, "Rank %"
        PRIu64
        " has events to process, begin trace read...\n", rank());

        while ((hasEvent != 0) && (evQ.size() == 0)) {
            if (OTF2_SUCCESS !=
                OTF2_Reader_ReadGlobalEvents(traceReader, traceGlobalEvtReader, 1, &eventsRead)) {
                fatal(CALL_INFO, -1, "Error reading global event on rank %"
                PRIu64
                "\n", rank());
            } else {
                verbose(CALL_INFO, 4, 0, "Read %"
                PRIu64
                " events from trace on rank: %"
                PRIu64
                "\n",
                    eventsRead, rank());
            }
        ...
        verbose(CALL_INFO, 4, 0, "Rank %"
        PRIu64
        " has completed its read period from trace.\n", rank());

        return false;
    } else {
        verbose(CALL_INFO, 4, 0, "Rank %"
        PRIu64
        " has no events to read in global trace. Motif will mark complete.\n", rank());
        return true;
    }
