...
void Ember3DCommDoublingGenerator::configure() {
    if ((peX * peY * peZ) != (unsigned) size()) {
        fatal(CALL_INFO, -1, "Processor decomposition of %"
        PRIu32
        "x%"
        PRIu32
        "x%"
...
bool Ember3DCommDoublingGenerator::generate(std::queue<EmberEvent *> &evQ) {
    if (0 == rank()) {
        verbose(CALL_INFO, 1, 0, "Motif executing phase %"
        PRIu32
        "...\n", phase);
    }
    ++phase;
    ...
    next_comm_rank = convertPositionToRank(peX, peY, peZ, myX + comm_delta, myY + comm_delta,
                                           myZ + comm_delta);
    if (next_comm_rank > -1) {
        enQ_isend(evQ, next_comm_rank, itemsThisPhase,
                  0, GroupWorld, &requests[next_request]);
