    ...
    namespace Ember {
        ...
        public:
            EmberAMRBinaryFile(char *amrPath, Output *out) :
                ...
                out->verbose(CALL_INFO, 8, 0, "Set mesh file seek to: %"
                PRIu64
                "\n", meshStartIndex);
                ...
                for (uint32_t i = 0; i < rankCount; ++i) {
                    ...
                    for (uint32_t j = 0; j < blocksOnNode; j++) {
                        ...
                        if (block_present != globalBlockMap->end()) {
                            output->fatal(CALL_INFO, -1, "Block ID: %"
                            PRIu32
                            " already in map.\n", blockID);
                        }
            ...
            void populateLocalBlocks(std::vector<Ember3DAMRBlock *> *localBlocks, uint32_t rank) {
                ...
                output->verbose(CALL_INFO, 16, 0, "Seek file offet: Base=%"
                PRIu64
                ", Rank=%"
                PRIu32
                ", Offset=%"
                ...
                output->verbose(CALL_INFO, 16, 0, "Rank Offset: %"
                PRIu64
                ", seeking in file...\n", rankBlocksIndex);
                ...
                output->verbose(CALL_INFO, 16, 0, "Rank has %"
                PRIu32
                " blocks on the the node.\n", blocksOnNode);
                ...
                for (uint32_t i = 0; i < blocksOnNode; ++i) {
                    ...
                    output->verbose(CALL_INFO, 32, 0, "Read Block: %"
                    PRIu32
                    " X-:%"
                    PRId32
                    ", X+:%"
