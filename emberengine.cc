...
EmberEngine::EmberEngine(SST::ComponentId_t id, SST::Params &params) :
    ...
    selfEventLink = configureSelfLink("self", "1ps",
                                      new Event::Handler<EmberEngine>(this,
                                                                      &EmberEngine::handleEvent));
...
EmberEngine::ApiMap EmberEngine::createApiMap(OS *os,
    ...
    while (1) {
        ...
        Hermes::Interface *api = dynamic_cast<Interface *>( owner->loadSubComponent(moduleName,
                                                                                    owner,
                                                                                    modParams));
...
void EmberEngine::issueNextEvent(uint64_t nanoDelay) {
    ...
    output.debug(CALL_INFO, 8, ENGINE_MASK, "Engine issuing next event with delay %"
    PRIu64
    "\n", nanoDelay);
