...
namespace SST {
    namespace Ember {
        ...
        public:
            ...
            int calcPE_size() {
                return (m_num_pes - m_PE_start) / stride() +
                       (((m_num_pes - m_PE_start) % stride()) ? 1 : 0);
            }
                ...
                switch (m_phase) {
                    ...
                    case 1:
                        ...
                        if (0 == m_my_pe) {
                            ...
                            printf("%d:%s: PE_start=%d logPE_stride=%d PE_size=%d\n", m_my_pe,
                                   getMotifName().c_str(), m_PE_start, m_logPE_stride, m_PE_size);
                        }
                        {
                    ...
                    case 2:
                        ...
                        for (int h = 0; h < m_PE_size; h++) {
                            ...
                            for (int i = 0; i < m_nelems; i++) {
                                m_src.at<TYPE>(h * m_nelems + i) =
                                    ((TYPE) (m_my_pe + 1) << shift) | i + 1;
                            }
                        }
                    ...
                    case 3:
                        if (imIn(m_my_pe)) {
                            ...
                            if (4 == sizeof(TYPE)) {
                                enQ_alltoall32(evQ, m_dest, m_src, m_nelems, m_PE_start,
                                               m_logPE_stride, m_PE_size, m_pSync);
                            } else {
                                enQ_alltoall64(evQ, m_dest, m_src, m_nelems, m_PE_start,
                                               m_logPE_stride, m_PE_size, m_pSync);
                            }
                        } else {
                            ret = true;
                    ...
                    case 4:
                        for (int h = 0; h < m_PE_size; h++) {
                            ...
                            for (int i = 0; i < m_nelems; i++) {
                                ...
                                assert(m_dest.at<TYPE>(h * m_nelems + i) ==
                                       (((TYPE) (pe + 1) << shift) | i + 1));
                            }
                        }
                        if (0 == m_my_pe) {
