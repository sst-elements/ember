    ...
    namespace Ember {
        ...
        public:
            EmberShmemFAM_Get2Generator(SST::Component *owner, Params &params) :
                EmberShmemGenerator(owner, params, "ShmemFAM_Get2"), m_phase(Init),
                m_groupName("MyApplication"), m_curBlock(0), m_rng(NULL) {
                m_totalBytes = (size_t) params.find<SST::UnitAlgebra>(
                ...
                m_partitionSize = (size_t) params.find<SST::UnitAlgebra>("arg.partitionSize",
                                                                         "16MiB").getRoundedValue();
                m_backed = (bool) (0 ==
                                   params.find<std::string>("arg.backed", "yes").compare("yes"));
                    ...
                    case Fini:
                        ...
                        if (m_my_pe == 0) {
                            ...
                            printf("%d:%s:  %zu bytes, %.3lf GB/s \n", m_my_pe,
                                   getMotifName().c_str(), bytes, (double) bytes / time);
                        }
                        return true;
                }
                return false;
        ...
        private:
            ...
            bool work(std::queue<EmberEvent *> &evQ) {
                ...
                for (int i = 0; i < m_getLoop && m_curBlock < m_numBlocks; i++) {
                    ...
                    verbose(CALL_INFO, 2, 0, "0x%"
                    PRIx64
                    " %p\n", m_mem.getSimVAddr(), m_mem.getBacking());
                    Hermes::MemAddr m_dest = m_mem.offset<unsigned char>(
                        m_blockSize * (m_curBlock % m_numBlocksPerPartition));
