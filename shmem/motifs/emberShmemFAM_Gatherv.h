    ...
    namespace Ember {
        ...
        public:
            EmberShmemFAM_GathervGenerator(SST::Component *owner, Params &params) :
                EmberShmemGenerator(owner, params, "ShmemFAM_Gatherv"), m_phase(Init),
            ...
            bool generate(std::queue<EmberEvent *> &evQ) {
                switch (m_phase) {
                    ...
                    case Work:
                        ...
                        if (m_blockStride) {
                            if (m_blocking) {
                                enQ_fam_gather_blocking(evQ, m_src, m_fd, m_numBlocks, m_firstBlock,
                                                        m_blockStride,
                                                        m_blockSize);
                            } else {
                                enQ_fam_gather_nonblocking(evQ, m_src, m_fd, m_numBlocks,
                                                           m_firstBlock, m_blockStride,
                                                           m_blockSize);
                            }
                        } else {
                            if (m_blocking) {
                                enQ_fam_gatherv_blocking(evQ, m_src, m_fd, m_numBlocks, m_indexes,
                                                         m_blockSize);
                            } else {
                                enQ_fam_gatherv_nonblocking(evQ, m_src, m_fd, m_numBlocks,
