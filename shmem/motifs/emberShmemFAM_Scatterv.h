    ...
    namespace Ember {
        ...
        public:
            EmberShmemFAM_ScattervGenerator(SST::Component *owner, Params &params) :
                EmberShmemGenerator(owner, params, "ShmemFAM_Scatterv"), m_phase(Init),
                m_groupName("MyApplication") {
                ...
                m_backed = (bool) (0 ==
                                   params.find<std::string>("arg.backed", "no").compare("yes"));
            ...
            bool generate(std::queue<EmberEvent *> &evQ) {
                switch (m_phase) {
                    ...
                    case Work:
                        ...
                        if (m_blockStride) {
                            if (m_blocking) {
                                enQ_fam_scatter_blocking(evQ, m_src, m_fd, m_numBlocks,
                                                         m_firstBlock, m_blockStride, m_blockSize);
                            } else {
                                enQ_fam_scatter_nonblocking(evQ, m_src, m_fd, m_numBlocks,
                                                            m_firstBlock, m_blockStride,
                                                            m_blockSize);
                            }
                        } else {
                            if (m_blocking) {
                                enQ_fam_scatterv_blocking(evQ, m_src, m_fd, m_numBlocks, m_indexes,
                                                          m_blockSize);
                            } else {
                                enQ_fam_scatterv_nonblocking(evQ, m_src, m_fd, m_numBlocks,
                                                             m_indexes, m_blockSize);
                            }
                        }
                        if (!m_blocking) {
                    ...
                    case Fini:
                        ...
                        if (m_my_pe == 0) {
                            ...
                            printf("%d:%s:  %zu bytes, %.3lf GB/s, postTime %"
                            PRIu64
                            " \n", m_my_pe, getMotifName().c_str(), bytes, (double) bytes /
                                                                           time, postTime );
                        }
                        return true;
                }
