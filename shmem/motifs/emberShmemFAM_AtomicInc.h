...
namespace SST {
    namespace Ember {
        ...
        public:
            EmberShmemFAM_AtomicIncBaseGenerator(SST::Component *owner, Params &params,
                                                 std::string name) :
                EmberShmemGenerator(owner, params, name), m_phase(-3), m_one(1), m_numFamNodes(0) {
                m_computeTime = params.find<int>("arg.computeTime", 0);
                m_totalBytes = (uint64_t) params.find<SST::UnitAlgebra>(
                    "arg.totalBytes").getRoundedValue();
                ...
                if (-3 == m_phase) {
                    ...
                    if (0 == m_my_pe) {
                        printf("outerLoop done %d %f\n", m_outLoop,
                               ((double) m_iterations * (double) m_updates * (double) m_num_pes *
                                1.0e-9) / m_times[m_outLoop]);
                    }
                    ...
                    if (0 == m_my_pe) {
                        double updateTotal = ((double) m_iterations * (double) m_updates *
                                              (double) m_num_pes);
...
#if USE_SST_RNG
                retval =
                    ((uint64_t) m_rng->generateNextUInt32()) << 32 | m_rng->generateNextUInt32();
#else
        ...
        template<class TYPE, int VAL>
        class EmberShmemFAM_AtomicIncGenerator
            : public EmberShmemFAM_AtomicIncBaseGenerator<TYPE, VAL> {
        public:
            EmberShmemFAM_AtomicIncGenerator(SST::Component *owner, Params &params,
                                             std::string name) :
        ...
        template<class TYPE>
        class EmberShmemFAM_AtomicIncGenerator<TYPE, 1>
            : public EmberShmemFAM_AtomicIncBaseGenerator<TYPE, 1> {
        public:
            EmberShmemFAM_AtomicIncGenerator(SST::Component *owner, Params &params,
                                             std::string name) :
        ...
        template<class TYPE>
        class EmberShmemFAM_AtomicIncGenerator<TYPE, 2>
            : public EmberShmemFAM_AtomicIncBaseGenerator<TYPE, 2> {
        public:
            EmberShmemFAM_AtomicIncGenerator(SST::Component *owner, Params &params,
                                             std::string name) :
        ...
        class EmberShmemFAM_AtomicIncIntGenerator
            : public EmberShmemFAM_AtomicIncGenerator<int, 0> {
        public:
        ...
        class EmberShmemFAM_AtomicIncLongGenerator
            : public EmberShmemFAM_AtomicIncGenerator<long, 0> {
        public:
            SST_ELI_REGISTER_SUBCOMPONENT(
