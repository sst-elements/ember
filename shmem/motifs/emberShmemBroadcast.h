        ...
        public:
            ...
            bool generate(std::queue<EmberEvent *> &evQ) {
                ...
                switch (m_phase) {
                    ...
                    case 3:
                        if (8 == sizeof(TYPE)) {
                            enQ_broadcast64(evQ, m_dest, m_src, m_nelems, m_root, 0, 0, m_num_pes,
                                            m_pSync);
                        } else {
                            enQ_broadcast32(evQ, m_dest, m_src, m_nelems, m_root, 0, 0, m_num_pes,
                                            m_pSync);
                        }
                        break;
                        ...
                        for (int i = 0; i < m_nelems; i++) {
                            ...
                            if (m_my_pe != m_root) {
                                if (m_printResults) {
                                    printf("%d:%s: %#"
                                    PRIx64
                                    "\n", m_my_pe, getMotifName().c_str(), (uint64_t) m_dest.at<TYPE>(
                                        i));
                                }

                                assert(
                                    (((TYPE) (m_root + 1) << shift) | i + 1) == m_dest.at<TYPE>(i));
                            }
                        }
                        ret = true;
                        if (0 == m_my_pe) {
